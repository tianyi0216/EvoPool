...
it diagnoses WHERE the current pool fails and WHY.

Key analyses:
  A. Pool overview            -- coverage, per-class recall, vote depth
  B. Per-annotator diagnostics -- marginal coverage, redundancy, overlap
  C. Coverage gap             -- class breakdown, feature-segment analysis
  D. Fragility                -- examples with only 1 annotator firing
  E. Error analysis           -- FP/FN patterns, conflicts
  F. Improvement targets      -- scored + ranked clusters with sampled examples
"""
    ...
    *,
    ...
    for c in centers:
        ...
        for i in idxs:
            lab = int(agg_label[i])
            if lab != ABSTAIN:
                _label_counts[lab] = _label_counts.get(lab, 0) + 1
        if _label_counts:
            _tgt = max(_label_counts.items(), key=lambda kv: kv[1])[0]
        else:
            _tgt = int(agg_label[c])  # may itself be ABSTAIN
        _tgt_name = (LABEL_NAMES.get(_tgt, "abstain")
                     if _tgt != ABSTAIN else "abstain")
        ...
        clusters.append({
            ...
            "target_label": _tgt if _tgt != ABSTAIN else None,
            "target_label_name": _tgt_name,
