#!/usr/bin/env python3
"""EvoPool: multi-label error analysis -> improvement_brief.json.
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--task_name", type=str, default="pubmed",
        help="Task name (pubmed for multi-label). Sets label names and features.",
    )
    p.add_argument(
        "--pool_module", type=Path,
        default=Path("annotators/pubmed/initial_pool.py"),
    )
    p.add_argument(
        "--data_path", type=Path,
    ...
    p.add_argument(
        "--out_path", type=Path,
        default=Path("runs/pubmed/initial/improvement_brief.json"),
    )
